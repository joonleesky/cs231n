...
def affine_batchnorm_relu_forward(x, w, b, gamma, beta, bn_param):
    ...
    Returns a tuple of:
    - out: Output from the bathcnorm ReLU
    - cache: Object to give to the backward pass
    """
    a, fc_cache = affine_forward(x, w, b)
    x_hat, batch_cache = batchnorm_forward(a, gamma, beta, bn_param)
...
def affine_batchnorm_relu_backward(dout, cache):
    ...
    dx_hat = relu_backward(dout, relu_cache)
    da, dgamma, dbeta = batchnorm_backward(dx_hat, batch_cache)
    dx, dw, db = affine_backward(da, fc_cache)
...
def affine_relu_dropout_forward(x, w, b, dropout_param):
    ...
    Returns a tuple of:
    - h: Output from the ReLU
    - out: Output after the dropout
    - cache: Object to give to the backward pass
    """
    a, fc_cache = affine_forward(x, w, b)
    h, relu_cache = relu_forward(a)
    out, drop_cache = dropout_forward(h, dropout_param)
...
def affine_relu_dropout_backward(dout, cache):
    ...
    dh = dropout_backward(dout, drop_cache)
...
def affine_batchnorm_relu_dropout_forward(x, w, b, gamma, beta, bn_param, dropout_param):
    a, fc_cache = affine_forward(x, w, b)
    x_hat, batch_cache = batchnorm_forward(a, gamma, beta, bn_param)
    h, relu_cache = relu_forward(x_hat)
    out, drop_cache = dropout_forward(h, dropout_param)
...
def affine_batchnorm_relu_dropout_backward(dout, cache):
    ...
    dh = dropout_backward(dout, drop_cache)
    dx_hat = relu_backward(dh, relu_cache)
    da, dgamma, dbeta = batchnorm_backward(dx_hat, batch_cache)
    dx, dw, db = affine_backward(da, fc_cache)
    return dx, dw, db, dgamma, dbeta
