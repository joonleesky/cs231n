...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        feature_cache = (features, W_proj, b_proj)
        ...
        score, pred_cache = temporal_affine_forward(h, W_vocab, b_vocab)
        loss, dscore = temporal_softmax_loss(score, captions_out, mask)
            
        #backward pass
        dh, dW_vocab, db_vocab = temporal_affine_backward(dscore, pred_cache)
        if self.cell_type == 'rnn':
        ...
        _, dW_proj, db_proj = affine_backward(dh0, feature_cache)
    ...
    def sample(self, features, max_length=30):
        ...
        h0 = np.dot(features, W_proj) + b_proj
        c0 = np.zeros_like(h0)
        start = np.tile(W_embed[self._start],(N,1))  #<start>
